...
const baseRegisterSchema = z.object({
  ...
  israeliId: z.string().optional().refine(
    (id) => !id || validateIsraeliId(id),
    { message: 'رقم الهوية الإسرائيلية غير صحيح' }
  ),
  businessId: z.string().optional().refine(
    (id) => !id || validateBusinessId(id),
    { message: 'رقم سجل الأعمال غير صحيح' }
  ),
  termsAccepted: z.boolean().refine(v => v === true, { message: 'يجب الموافقة على الشروط' }),
});
...
export function AuthPage() {
  ...
  const onLogin = async (data: any) => {
    ...
    try {
      ...
      const msg = e?.response?.data?.message || 'خطأ في تسجيل الدخول';
      setLoginError(msg);
    }
  };
  ...
  const onRegister = async (data: any) => {
    ...
    try {
      ...
      const msg = e?.response?.data?.message || 'خطأ في إنشاء الحساب';
              ...
              {isLogin ? (
                ...
                >
                  ...
                  <p className="text-center text-muted-foreground mt-4">
                    {t('noAccount')}{' '}
                    <button type="button" onClick={() => { setIsLogin(false); setLoginError(''); }} className="text-accent font-bold hover:underline">
                      {t('register')}
                    </button>
                  </p>
                </motion.form>
                ...
                >
                  ...
                  <div className="flex gap-2 p-1 bg-secondary rounded-xl">
                    {(['worker', 'employer'] as const).map((role) => (
                      <button
                        ...
                        className={`flex-1 py-3 rounded-lg text-sm font-bold transition-all ${registerForm.watch('role') === role
                  ...
                  {/* Israeli ID with live validation */}
                  <div className="space-y-1">
                    <div className="relative">
                      <Input
                        placeholder={t('israeliIdOptional')}
                        inputMode="numeric"
                        maxLength={9}
                        {...registerForm.register('israeliId')}
                        error={registerForm.formState.errors.israeliId?.message as string}
                      />
                    ...
                    {israeliIdStatus === 'valid' && (
                      <p className="text-xs text-green-600 px-1">✓ {t('israeliId')} صحيح</p>
                    )}
                  ...
                  {/* Business ID for employers only */}
                  {selectedRole === 'employer' && (
                    <div className="space-y-1">
                      <div className="relative">
                        <Input
                          placeholder={t('businessIdOptional')}
                          inputMode="numeric"
                          maxLength={9}
                          {...registerForm.register('businessId')}
                          error={registerForm.formState.errors.businessId?.message as string}
                        />
                  ...
                  {registerForm.formState.errors.termsAccepted && (
                    <p className="text-xs text-destructive">{registerForm.formState.errors.termsAccepted.message as string}</p>
                  )}
                  ...
                  <p className="text-center text-muted-foreground mt-2">
                    {t('hasAccount')}{' '}
                    <button type="button" onClick={() => { setIsLogin(true); setRegisterError(''); }} className="text-accent font-bold hover:underline">
                      {t('login')}
                    </button>
                  </p>
                </motion.form>
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="h-1.5 bg-secondary rounded-full overflow-hidden">
        <div
          className="h-full bg-gradient-to-l from-accent to-primary rounded-full transition-all duration-500"
          style={{ width: `${pct}%` }}
        />
      </div>
    </div>
